...
| Mobile | badge/confidence rendering; absent-control logic | — | generated client | native UI tests (Compose/Robolectric; iOS models + simulator): progress/failure/refund states; consent flow fail-closed | Maestro: generate view → badge visible → replace with real photo → supersession; try-on avatar-base flow |
...
- **AC-4:** metadata schema requires `provenance` + `confidence` on every generated asset (contract test rejects absence); UI badge visible and screen-reader-announced on both features (UI-test + Maestro artifacts); every `garment_representations` row carries its G-level with no conflation (schema test) — REQ-CAP-080/100.
