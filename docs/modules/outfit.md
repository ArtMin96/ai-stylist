# Module Contract — `outfit`

> Module names are canonical per [SPINE §3](../../planning/SPINE.md). Path: `apps/api/src/modules/outfit`. This contract is the module's source of truth; code that contradicts it is wrong until a DEC entry says otherwise.

- **Responsibility (one sentence):** Garment representation levels (G0–G4), outfit composition, and saved outfits.
- **Owner:** @team (placeholder — see `CODEOWNERS`) · **Status:** skeleton (P02) · **Last updated:** 2026-09-13

## Public interface

Public interface: `index.ts` only. P02 skeleton: the sole export is the empty NestJS `OutfitModule` class (composition-root wiring, imported by `tests/outfit.smoke.test.ts`); no domain service, command, query, type, or port is exported yet. Everything under `internal/` is blocked by the `public-api-only` boundary rule (`just arch-check`, `just lint`).
...
None yet (P02 skeleton). Planned per SPINE §3: `outfits`, `outfit_items`, `garment_representations`. Table definitions, when they exist, live in `apps/api/src/modules/outfit/internal/schema.ts` and are composed from `packages/db/` (ADR-0001 §3).
...
- Public API of `closet`; `packages/shared-kernel`. Consumed by `recommendation` for item/composition **types** only.
...
- Exposes item/composition types to `recommendation` without any presentation or 3D/asset types on that edge (doc 04 §4.2 rule 2).

## Tests

- Location: `apps/api/src/modules/outfit/tests/`
...
- New garment representation levels are added to the G-ladder (SPINE §4) and `shared-kernel` capability codes, not as parallel models.
