...
| `recommendation-not-renderer`      | `modules/recommendation/**` importing `modules/avatar`, `apps/mobile/src/render`, `react-native-filament`, `assets/`, or `.glb/.gltf/.ktx2`                                                                                   |
...
| `domain-no-provider-sdk`           | `modules/**` importing `@trigger.dev/*`, `@aws-sdk/*`, `@cloudflare/*`, `react-native-purchases`, `@fal-ai/*`, `posthog-*`, `firebase-admin`, `@sentry/*`                                                                     |
...
| `render-boundary`                  | `apps/mobile/**` outside `src/render/**` and `src/features/avatar/**` importing `src/render/**` or `react-native-filament`                                                                                                    |
| `mobile-workers-not-server`        | `apps/mobile/**` importing workspace code other than `packages/contracts` and `packages/shared-kernel`                                                                                                                        |
| `composition-root-only`            | any file except the composition roots (`app.module.ts`, `main.ts`, `trigger/**`, `_layout.tsx`, `app-services.tsx`), `apps/api/tests/**`, `platform` itself, `dev`, and the seed CLI importing `platform/**` or `packages/db` |
...
- `tsconfig.json` here exists only to hand the mobile `@/*` alias to dependency-cruiser; every other
