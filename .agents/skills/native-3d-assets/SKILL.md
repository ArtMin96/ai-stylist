...
description: Work on the 3D layer — the Filament boundary in apps/mobile/src/render/, parametric avatar meshes/morphs, poses, garment representations G0–G4, the glTF/KTX2 asset pipeline under assets/3d/, and manifests — including real-device validation.
---
...
- Any change under `apps/mobile/src/render/`, `react-native-filament` usage, designated 3D screens, avatar morph/pose logic, garment attachment, or any file under `assets/3d/`.
...
2. `assets/3d/` manifest schema and the morph-target/skeleton name registry (P02-T17 ships the schema; P01 conventions ratify it — see planning/16 RISK-01/OQ-08 for the device floor).
3. `apps/mobile/src/render/README.md` (boundary directory; empty in P02) and `docs/modules/avatar.md`, `docs/modules/outfit.md`.
...
2. Renderer isolation (`render-boundary`): render code consumes structured outfit/avatar descriptions from `packages/contracts`; it never imports recommendation, profile, or closet logic, and nothing outside `apps/mobile/src/render/` except designated 3D screens imports Filament types. `recommendation` never imports anything here.
...
```bash
just assets-validate                  # glTF validity, KTX2, budgets, manifest schema, morph names (KTX2/budget checks TODO per brief §8 #13)
...
- `assets-validate` depends on P01 tooling that is `NOT_STARTED` → ship what exists, open an issue for the rest; never claim a check ran that did not.
