...
**Status:** Draft for ratification · **Date:** 2026-08-24 · **Amended:** 2026-09-09 (prices re-verified; §2.4–2.5, §3, §5, §6.1 re-baselined — [r6](research/r6-pricing-verification-2026-09-09.md), DEC-34) · **Amended:** 2026-09-13 ([r7](research/r7-third-party-services-and-self-hosting-audit-2026-09-13.md), ADR-0003 — LLM explanation polish removed (DEC-46); self-hosted eval arms BiRefNet, Qwen3-VL, SigLIP/SigLIP2, FASHN VTON v1.5 added (DEC-47); §2.7, §3, §5, §6, §7 updated) · **Owner:** AI/ML architecture
...
| 8 | Recommendation explanations | NL | No | Templates from structured reason codes only; LLM wording polish removed from the product path (DEC-46) |
...
- **Caching/dedup/batch:** inputs are content-hashed **before** any paid call; identical hash + same model/prompt version → serve stored result, never reprocess. Jobs are idempotent (idempotency key = `hash + task + model_version`, enforced by pg-boss singleton keys). Derived results are stored with lineage and invalidated only when their specific input or model version changes. Async work uses provider Batch APIs (−50%) wherever the user is not waiting.
...
| Provider → fallback | **Apple Vision subject lift (iOS) / ML Kit Subject Segmentation (Android), $0** → server fallback for failures/low-confidence, **evaluated in P06 between self-hosted BiRefNet (MIT weights) in the segmentation worker and BiRefNet v2 on fal.ai** ($0.0008/compute-s ≈ $0.004–0.008/image; Bria RMBG 2.0 $0.018/image if a commercial licence is required) — the existing IoU/latency/cost gate decides (DEC-47). Never "rembg defaults": its RMBG-2.0 weights are BRIA-licensed. remove.bg rejected ($0.20–1.00/image). r6 §1.3 |
...
| Provider → fallback | **Vision-LLM structured extraction: Gemini 3.5 Flash ($0.30/$2.50 per M tok, ≈ $0.0012/item standard, $0.00075 batch) or Claude Haiku 4.5 ($1/$5 per M, ≈ $0.0037/item, $0.00185 batch) plus self-hosted Qwen3-VL (Apache-2.0) as an explicit eval arm (macro-F1, schema-valid output, latency, GPU cost, privacy) — pick per eval in P06 (DEC-47)**; cheap coarse pass: **Google Cloud Vision labels $1.50/1k**; escalation: **Ximilar Fashion Tagging** (custom quote, est. $0.003–0.01/img) only if eval precision insufficient. r3 §2.2, SPINE §2 |
...
| Provider → fallback | **Voyage multimodal-3.5 (per-pixel, ≈ $0.0003 per 1 MP image; 200M free tokens)** or Cohere Embed v4 ($0.47/1M image tokens; tokens-per-image undocumented — get a datasheet before committing) ; **self-hosted SigLIP/SigLIP2 (Apache-2.0) is a P06 eval arm from the start** (batch/async embedding suits self-hosting — DEC-47), with CLIP/Nomic Embed Vision as further self-hosted variants (H100 $1.89/hr discounted on fal, or an owned GPU host). r6 §2 |
...
| Provider → fallback | **fal.ai FLUX.2 [dev] with reference image ($0.012/MP, warm pool, seconds-class latency; licensed managed endpoint only — FLUX.2 [dev] weights are non-commercial, never self-hosted)**; FLUX.1 Schnell ($0.003/MP) if eval quality allows; escalation FLUX Kontext [pro] ($0.04/image) only if FLUX.2 dev fails the SPK-2 gate → Gemini 2.5 Flash Image ($0.039) / Replicate batch-only for backfill. r6 §1.2 |
...
| Provider → fallback | **fal.ai FASHN try-on v1.6 ($0.075/generation) or Kling Kolors try-on ($0.07)** — ~100ms-class queue latency; **cost arm of the P11 gate: FLUX 2 try-on LoRA ($0.021/MP) evaluated on the same set — adopt if it passes (managed endpoint only: FLUX.2 [dev] weights are non-commercial for self-hosting); self-hosted FASHN VTON v1.5 (Apache-2.0 code + weights) is a further P11 gate arm (DEC-47)** → Replicate try-on for batch/backfill. Leffa ($0.10) is the price ceiling. r6 §1.1 |
...
### 2.7 Explanations for recommendations (templates only — LLM polish removed, DEC-46)
...
| Why not deterministic | It is: **templates over reason codes are the product path.** LLM wording polish was removed 2026-09-13 (DEC-46); any future NL polish needs a new entry in this doc + eval before it can ship |
| Provider → fallback | **None — deterministic templates, $0.** Historical candidates (Claude Haiku 4.5 batch+cache ≈ $0.0001–0.0006/explanation; Gemini 3.5 Flash ≈ $0.00033) stay on record in r6 §2 for any future re-evaluation |
| On-device vs server | Server, rendered inline with the recommendation |
| Quality threshold | Every sentence maps to a real reason code by construction (one template per code, typed params); the faithfulness check in doc 13 §10 stays as the release gate |
| Eval | 200 decision traces incl. edge cases (cold-weather holiday, laundry conflicts, sparse closet); metrics: faithfulness (no unsupported claims) 100% by construction, verified on the set; per-locale readability review of templates |
| Latency / cost | Rendered with the rec; $0 |
| Caching | Not needed — template rendering is cheap; the reason-code set is cached with the recommendation |
| Degraded mode | None required — templates are the only path; users always get full explanations |
| Version tracking | Template registry: `explanation-templates@vN` recorded per rendered text (doc 09) |
...
| | BiRefNet v2 / Bria RMBG 2.0 on fal.ai | ●●● | ~$0.006 / $0.018 per img | ●●○ | low (open weights) | Server fallback candidate (≤10% of items) — P06 gate vs self-host |
| | Self-hosted BiRefNet (MIT weights) in the segmentation worker | ●●● | share of owned host (GPU only if needed) | ●●● stays in our infra | ●○○ open weights | P06 eval arm vs the fal.ai fallback (DEC-47) |
...
| | Self-hosted Qwen3-VL (Apache-2.0) | ●●○ to verify | owned GPU host share | ●●● stays in our infra | ●○○ open weights | P06 eval arm: macro-F1, schema-valid output, latency, GPU cost, privacy (DEC-47) |
...
| | Self-hosted SigLIP/SigLIP2 (Apache-2.0; CLIP/Nomic variants) | ●●○ | GPU ~$1.10–1.89/hr or owned GPU host | ●●● | ●○○ | P06 eval arm from the start — batch/async suits self-hosting (DEC-47) |
| | Cohere Embed v4 | ●●● | $0.47/1M img tok; tokens/img undocumented | ●●○ | ●●○ | Fallback once token rule is confirmed |
| Generative try-on | **fal.ai FASHN v1.6 / Kling Kolors** | ●●● | **$0.075 / $0.07 per img** | ●●○ needs doc 11 review | ●●○ | **PICK** — warm pool; FLUX 2 try-on LoRA ($0.021/MP, managed endpoint only — FLUX.2 [dev] weights non-commercial) is the P11 cost-arm candidate |
| | Self-hosted FASHN VTON v1.5 (Apache-2.0 code + weights) | ●●○ to verify | owned GPU host | ●●● stays in our infra | ●○○ open weights | P11 gate arm (DEC-47) |
| Missing views | **fal.ai FLUX.2 [dev]** | ●●● | $0.012/MP (Schnell $0.003; Kontext pro $0.04) | ●●○ | ●●○ | **PICK** (licensed managed endpoint; [dev] weights are non-commercial for self-hosting) |
...
| NL trend summaries (explanations are templates — LLM polish removed, DEC-46) | **Claude Haiku 4.5 (batch+cache)** | ●●● | $1/$5 per M; ~$0.0001/user/mo amortized | ●●● 7-day, never trains | ●○○ prompts portable | **PICK** for trend summarization (privacy anchor); not used for explanations |
| | OpenAI nano-class | ●●○ | ~$0.15/$0.60 per M (pricing page unreachable 2026-09-09 — low confidence) | ●●○ 30-day | ●○○ | Fallback |
| | Gemini 3.5 Flash | ●●○ | $0.30/$2.50 per M; ≈ $0.00033/summary | ●●○ | ●○○ | Cheaper co-pick if privacy review allows |
| Face likeness | **ARKit/MediaPipe (on-device)** | ●●○ stylized | $0 | ●●● | ●○○ | **PICK** — see §2.6 |

One-line answer for the product owner: **no single provider wins everything.** The cost+quality frontier is: **on-device Apple/Google CV for pixels ($0) · Gemini-Flash-class or Claude-Haiku-class for structured vision extraction (eval decides in P06) · Voyage for embeddings · fal.ai for all generative images (FASHN/Kling for try-on at $0.07–0.075, FLUX.2 dev for views at $0.012) · templates for explanations ($0, DEC-46) · Claude Haiku (batch+cache) for trend summaries.** Self-hosted arms (BiRefNet, Qwen3-VL, SigLIP/SigLIP2 in P06; FASHN VTON v1.5 in P11) enter the same gates (DEC-47). Try-on is the only expensive unit — it is ~35× a closet item and drives the weighted-credit model in doc 12. Anthropic is the privacy anchor (7-day retention, never trains on API data); fal.ai is the only latency-viable generative host.

---

## 4. Cross-cutting cost mechanics

- **Batch −50%** (Anthropic/OpenAI/Google) on everything a user isn't waiting for: classification, embeddings, trend summaries.
...
| Cost per NL explanation | **$0** — templates from reason codes (DEC-46) | no LLM in the explanation path |
...
- 85% → page + automatic tightening: generative jobs shift to batch/off-peak, trend-summary cache TTL extended.
...
1. Trend summarization paused (feed serves existing summaries). Explanations are already templates (DEC-46) and have no rung — users always get full explanations.
2. Vision-LLM extraction → Cloud Vision coarse labels + user completes attributes.
3. Server segmentation fallback off → on-device only; failures queue for later.
4. Missing-view synthesis paused (credits not consumed; "temporarily unavailable").
5. G2 try-on paused → G0 collage fallback (credits not consumed).

Rungs 4–5 are also the automatic response to provider outage. Nothing on the ladder ever breaks capture, closet, or recommendations — the deterministic core runs at $0 marginal AI cost.
...
Known migration triggers already on file: Gemini 2.5 Flash sunset Oct 16 2026; Anthropic pricing review Sep 1 2026; embeddings self-host evaluated as a P06 arm rather than at a volume threshold (DEC-47).
...
| ID | Item | Owner phase |
|---|---|---|
| AIC-O1 | Final vision-LLM pick (Gemini Flash-class vs Claude Haiku vision-class vs **self-hosted Qwen3-VL**, amended 2026-09-13 — DEC-47) by eval bake-off: macro-F1, schema-valid output, latency, GPU cost, privacy | P06 |
...
| AIC-O5 | P11 gate arms: FLUX 2 try-on LoRA ($0.021/MP, licensed managed endpoint only — non-commercial weights) vs FASHN v1.6 ($0.075) vs **self-hosted FASHN VTON v1.5** (amended 2026-09-13 — DEC-47) on the same eval set; if a cheaper arm passes, revisit credit weights (DEC-34) | P11 |
| AIC-O6 | Re-verify all r6 prices at P11 and P13 kickoff (fal.ai model pages, Gemini/Anthropic pricing, store fees) | P11, P13 |
| AIC-O7 | Embedding provider: confirm Cohere tokens-per-image or lock Voyage multimodal-3.5; **self-hosted SigLIP/SigLIP2 evaluated as a P06 arm alongside** (amended 2026-09-13 — DEC-47) | P06 |
