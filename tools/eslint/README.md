...
| `quality/test-placement`      | `rules/test-placement.mjs`      | error    | `*.test.*` / `*.spec.*` must sit under a `tests/` directory; exceptions `apps/mobile/e2e/**`, `apps/api/tests/**` (HTTP tests + `tests/migrations/**`)       |
...
| `no-console`                  | `quality.mjs`                   | error    | everywhere in `apps/**`, `packages/**`, `tools/**` except `**/scripts/**`, `tools/codegen/**` and generated code — log through pino (API) or a port (mobile) |
