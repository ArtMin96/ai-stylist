...
- **Owner:** @team (placeholder — see `CODEOWNERS`) · **Status:** skeleton (P02) · **Last updated:** 2026-09-26
...
| `STORAGE_PROVIDER`, `StorageProvider`, `InMemoryStorageProvider`, `StorageError`, `Presign*` types, `StorageNamespace`, `MAX_PRESIGN_GET_TTL_SECONDS`, `MAX_PRESIGN_PUT_TTL_SECONDS`, `clampTtl` | port / adapter                         | Object storage with presigned URLs; R2 adapter pending (P02-T13)                                                                                                      |
...
3. Presigned URLs are short-lived (doc 11 §5.4): GET (read) TTL is clamped to ≤ 10 minutes (`MAX_PRESIGN_GET_TTL_SECONDS`), PUT/multipart (write) TTL to ≤ 15 minutes (`MAX_PRESIGN_PUT_TTL_SECONDS`); asserted in `apps/api/src/platform/tests/storage.port.test.ts`.
