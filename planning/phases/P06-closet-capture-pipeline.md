...
| Mobile | Queue drain logic, quality-check thresholds | Mutation log: replay after crash reproduces identical queue state | Generated client compile | native UI tests (Compose/Robolectric; iOS models + simulator): capture/review/correction flows | Maestro: single + batch capture (camera mocked in emulator, real on device); **airplane-mode ≥5-item capture → full sync on reconnect; kill-app-mid-upload resumes (REQ-CAP-130)**; a11y pass on capture + review |
