# Module Contract — `fashion-intel`

> Module names are canonical per [SPINE §3](../../planning/SPINE.md). Path: `apps/api/src/modules/fashion-intel`. This contract is the module's source of truth; code that contradicts it is wrong until a DEC entry says otherwise.

- **Responsibility (one sentence):** Trend, runway, and seasonal content ingestion with provenance, and personalization of the feed.
- **Owner:** @team (placeholder — see `CODEOWNERS`) · **Status:** skeleton (P02) · **Last updated:** 2026-09-13

## Public interface

Public interface: `index.ts` only. P02 skeleton: the sole export is the empty NestJS `FashionIntelModule` class (composition-root wiring, imported by `tests/fashion-intel.smoke.test.ts`); no domain service, command, query, type, or port is exported yet. Everything under `internal/` is blocked by the `public-api-only` boundary rule (`just arch-check`, `just lint`).
...
None yet (P02 skeleton). Planned per SPINE §3: `content_items`, `sources`, personalization signals. Table definitions, when they exist, live in `apps/api/src/modules/fashion-intel/internal/schema.ts` and are composed from `packages/db/` (ADR-0001 §3).
...
- Public APIs of `profile` and `closet`; `packages/shared-kernel`. Consumed by `assistant`.
...
- Content sources are OPEN — see planning/16 OQ-05; no source integration lands without that answer and a provenance record per item.

## Tests

- Location: `apps/api/src/modules/fashion-intel/tests/`
...
- New content sources register behind an ingestion port with a provenance schema owned by `packages/contracts`.
