...
**Status:** Ratified for planning · **Date:** 2026-08-24 · **Amended:** 2026-09-09 (§2 fal.ai prices, §6 weighted credits — [r6](research/r6-pricing-verification-2026-09-09.md), DEC-34/35) · 2026-09-13 (§2 self-hosting baseline: jobs, hosting, database, storage delivery, holidays, AI eval arms; §6 Google Play fee — [r7](research/r7-third-party-services-and-self-hosting-audit-2026-09-13.md), [ADR-0003](../docs/adr/0003-self-hosted-infrastructure-baseline.md), DEC-41–48) · 2026-09-22 (§1 team/Mac, §2 mobile framework, 3D renderer, API contract, push, iOS build rows, §5 P01: native iOS/Android replace React Native + Expo — [ADR-0004](../docs/adr/0004-native-ios-and-android-clients.md), DEC-49–54)
...
- **Team:** 2–3 developers, Linux workstations, heavy AI-coding-agent (Claude Code) usage. **One Mac for iOS development** (since 2026-09-22, DEC-51; it supersedes "no Mac owned"); GitHub Actions macOS runner for iOS CI.
- **User decisions already made (2026-08-24, from product owner):**
  ...
  4. Linux-first development; no Mac purchase planned — hosted macOS CI path. *(Superseded 2026-09-22 by DEC-51: the team has a Mac, and iOS work needs it.)*
  5. *(2026-09-22)* Fully native clients, Swift/SwiftUI and Kotlin/Compose, in this monorepo (DEC-49, [ADR-0004](../docs/adr/0004-native-ios-and-android-clients.md)).

## 2. Ratified technology decisions

| Area | Decision | Rejected / fallback | Evidence |
|---|---|---|---|
| Mobile framework | **Native: Swift 6 + SwiftUI (`apps/ios/`, XcodeGen + local SwiftPM packages) and Kotlin + Jetpack Compose (`apps/android/`, own Gradle root)**, sharing the backend through the OpenAPI contract; strict compiler/lint safety settings are part of the decision (DEC-49, 2026-09-22) | React Native + Expo superseded (DEC-04/38 → DEC-49); Kotlin Multiplatform rejected for now (Swift Export Alpha); Flutter (immature 3D) | ADR-0004, r2 |
| 3D renderer | **Deferred: no 3D in the native foundation.** When 3D resumes: **Google Filament's C++ engine used directly** on both platforms (Metal on iOS, official Android AARs); glTF 2.0, morph targets, skeletal animation, PBR (DEC-50) | `react-native-filament` superseded (dormant wrapper, RISK-01 retired); RealityKit rejected for the avatar path (USDZ-only, converters decode Draco and flatten KTX2); SceneKit dead; Unity-as-a-Library rejected (size/license) | ADR-0004, r2 |
...
| API contract | **OpenAPI 3.1 as the canonical contract**, with generated clients committed under `packages/contracts/gen/`: TS (API and tests), Swift (iOS) and Kotlin (Android) (DEC-53); contract files owned by `packages/contracts` | tRPC rejected: couples mobile to server internals, weaker fit for future chat/admin/3rd-party clients — documented in 05 | r4 + orchestrator reconciliation |
| Database | **Self-managed PostgreSQL 17 + pgvector** (Docker `pgvector/pgvector:pg17` on an owned host; pgBackRest PITR to encrypted off-host bucket, nightly `pg_dump`, PgBouncer, backup-age alerts, quarterly restore drill), **Drizzle ORM** + drizzle-kit migrations; ephemeral/Testcontainers databases for migration testing | Neon superseded (DEC-43; managed recovery was its only real value); Prisma (heavier), RDS (cost/ops at this scale); managed PostgreSQL is the fallback if the recovery obligations (RISK-17) are not met | r7, r4 |
...
| Jobs/queue | **pg-boss v12** (MIT) on the app's PostgreSQL for durable pipelines (image/AI), idempotency keys, retries, DLQ semantics; job definitions in `apps/api/src/jobs/`; workers in the API process or a dedicated `jobs` process | Self-hosted Trigger.dev only if the P02-T08 acceptance suite (kill/retry, idempotency, DLQ, replay, per-user cancellation, deletion/export) proves pg-boss insufficient (DEC-41); Trigger.dev Cloud superseded; Temporal rejected (separate service) | r7, r4 |
| ML workers | **Python FastAPI services in Docker** on the owned servers (Coolify-deployed, same private network as API/jobs/DB), called via versioned JSON schemas; separately deployable | Managed PaaS if ops load proves too high (DEC-42) | r7, r4 |
| Object storage/CDN | **Cloudflare R2** (S3-compatible, zero egress) behind `StorageProvider`. Delivery: **private user media = presigned GET on the S3 endpoint (uncached, TTL ≤ 10 min)**; **public app/content assets only (3D bundles, manifests, app content) = R2 custom domain with Cloudflare cache**. Fixed derivatives (cutout, thumbnails, palette swatch) produced once by the media workers | Cloudflare Images removed (DEC-44); imgproxy only if dynamic transforms are ever needed; self-hosted object storage (SeaweedFS) deferred until redundancy/offsite backup/delivery are designed; S3+CloudFront rejected (egress cost ~70× for media-heavy app) | r7, r4 |
| API hosting | **Owned servers + Docker + Coolify** (Apache-2.0: deploys, HTTPS, health checks, rollbacks, per-env variables; secrets sops → Coolify; one ~€10–25/mo server class at launch; provider/region per OQ-07/OQ-14, Hetzner = working assumption) | Railway superseded (DEC-42; its private networking never included an external DB); managed PaaS is the fallback if ops load proves too high | r7, r4 |
...
| Push | **FCM + APNs direct** (server-side); platform-native registration client-side (UserNotifications on iOS, FCM SDK on Android, DEC-52) | OneSignal rejected (unneeded vendor); Expo notifications removed with Expo | r4 |
...
| Holidays | **Embedded `date-holidays` library** (ISC code, CC-BY-3.0 data; no account, no network call, no key) behind a `HolidayProvider` port — adapter `DateHolidaysHolidayProvider`, provider id `'date-holidays'` | Nager.Date hosted API superseded (DEC-45) — kept only as a cross-check fixture source in P08 tests; Nager.Date self-hosting rejected (licence key); Calendarific dropped | r7, r4 |
| AI: background removal | **On-device first** (Apple Vision subject lift / ML Kit) → server fallback **evaluated in P06 between self-hosted BiRefNet (MIT weights) in the segmentation worker and fal.ai BiRefNet/RMBG endpoints** (IoU/latency/cost gate; never "rembg defaults" — RMBG-2.0 weights are BRIA-licensed) | remove.bg rejected (per-image cost) | r3, r7 |
| AI: classification & attributes | **Vision-LLM structured extraction (Gemini 3.5 Flash default, Claude Haiku 4.5 alt) with JSON schema** plus **self-hosted Qwen3-VL (Apache-2.0) as an explicit P06 eval arm** (macro-F1, schema-valid output, latency, GPU cost, privacy); Google Cloud Vision for cheap coarse labels; escalate to Ximilar Fashion Tagging only if eval precision insufficient | — | r3, r7 |
| AI: embeddings | **Multimodal embedding API (Voyage multimodal-3.5 default, Cohere alt — DEC-35)** plus **self-hosted SigLIP/SigLIP2 (Apache-2.0) as a P06 eval arm from the start** (batch/async work suits self-hosting) | — | r3, r7 |
| AI: explanations | **Templates from structured reason codes only** — LLM wording polish removed from the product path (DEC-46); any future natural-language polish needs a new doc-10 entry + eval | Explanations are produced by the engine's decision trace, never hallucinated after the fact; Claude Haiku polish superseded | r3, r7 |
| AI: generative try-on / missing views | **fal.ai** (try-on $0.07–0.075/image FASHN/Kling, $0.021/MP FLUX 2 LoRA eval-gated; missing view $0.012/MP FLUX.2 dev, $0.003 Schnell; ~100ms-class latency — verified 2026-09-09, r6) plus **self-hosted FASHN VTON v1.5 (Apache-2.0 code + weights) as a P11 gate arm** (DEC-47). **FLUX.2 [dev] weights are non-commercial**: usable only through a licensed managed endpoint, never self-hosted for commercial use | Replicate rejected for real-time (cold starts); usable for batch | r3, r5, r7 |
...
| iOS build/delivery | **GitHub Actions macOS runner is the only iOS CI lane** (`.github/workflows/ios.yml`); iOS development on the team's Mac (Linux works for Core packages, lint, format and codegen); TestFlight upload via App Store Connect API from CI (still to be built, OQ-18) (DEC-51) | EAS Build removed with Expo (DEC-30 → DEC-51; OQ-04 resolved); Xcode Cloud = candidate second lane if the runner proves unreliable | ADR-0004, r1 |
...
Dependency rules: modules expose a small public API (`index.ts`); internals not importable (enforced by ESLint boundaries + dependency-cruiser in CI; inside the native apps, boundaries are SwiftPM target dependencies and the Gradle module-graph allow-list). Domain never imports provider SDKs; `platform` implements ports. `recommendation` must not depend on `avatar`/renderer. `assistant` may only call application services.
...
| P01 | `P01-3d-and-capture-prototype-gate` | Anny asset pipeline + measurement harness; native Filament vertical spike on real devices (morphs, 3–4 poses, rotate/zoom) when 3D resumes; camera→upload; go/no-go gate *(re-scoped 2026-09-22: the RN+Filament gate is moot, DEC-50)* | P00 |
...
Unit economics anchors (r6, verified 2026-09-09): per closet item processed ≈ $0.002; AI cost/user/mo steady ≈ $0.01–0.06 (light–medium), heavy ≈ $0.10–0.15, **excluding credits**; onboarding spike $0.10–0.30; infra ≈ $25–45/mo at launch (ADR-0003; re-baseline at P02 per OQ-14), ≈ $370–450/mo at 5k MAU as an upper-bound hypothesis (≈ $0.08 per active user). Store fee base case 15% (Apple SBP; Google Play 15% for auto-renewing subscriptions = 10% service + 5% billing fee in the EEA/UK/US program from 2026-06-30, 15% elsewhere — verified 2026-09-13, r7; base case unchanged). Doc 12 owns the full calculation tables. Experiments queued for P13: Free cap 40 vs 100 items (market clusters at 100), weekly Plus SKU (weekly plans ≈ 55% of category revenue per Adapty 2026), annual discount depth, credit quantities.
