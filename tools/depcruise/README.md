...
| `domain-no-provider-sdk`           | `modules/**` importing `pg-boss`, `@aws-sdk/*`, `@cloudflare/*`, `react-native-purchases`, `@fal-ai/*`, `posthog-*`, `firebase-admin`, `@sentry/*`                                                                         |
...
| `composition-root-only`            | any file except the composition roots (`app.module.ts`, `main.ts`, `jobs/**`, `_layout.tsx`, `app-services.tsx`), `apps/api/tests/**`, `platform` itself, `dev`, and the seed CLI importing `platform/**` or `packages/db` |
