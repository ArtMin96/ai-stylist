...
| REQ-MED-060 | Optimization must produce LODs, compressed textures (KTX2/Basis), compressed meshes (Draco/meshopt), thumbnails, and CDN publication with versioned manifests. | Delivery formats match SPINE §2 ratified formats; manifest version bumps invalidate caches safely (NFR-PERF-050). | `media`, `platform` | P06, P10 | 07 |
...
| REQ-CTX-050 | Locale-aware public holidays (Nager.Date behind `HolidayProvider` port) must be provided, with the user choosing whether a holiday matters. | Holiday facts appear per locale; a per-recommendation toggle includes/excludes the holiday signal and defaults are user-configurable. | `context` | P08 | 09 |
...
| ID | Requirement | Acceptance criterion | Module | Phase(s) | Doc |
|---|---|---|---|---|---|
| REQ-EXP-010 | Each recommendation must state concise, concrete reasons (weather suitability, occasion fit, color harmony, rarely-worn item, saved preference) produced from the engine's decision trace as stable reason codes — never hallucinated afterward. | Every displayed reason maps 1:1 to a reason code emitted by a pipeline stage; reason-code registry lives in `shared-kernel`; NL polish (if any) is generated from codes only (NFR-AIC-040). | `recommendation`, `shared-kernel` | P09 | 09 |
...
| NFR-PERF-050 | CDN and asset caching must have safe invalidation and client disk limits. | Versioned manifests guarantee stale assets are never served after invalidation; client cache respects its disk cap under test. | `media`, `platform` | P06, P10 | 13 |
...
| NFR-AIC-040 | Small/specialized models must be tried before large general models, and natural-language explanations must be generated from structured reason codes with templates as the default (LLM polish optional, batch + prompt-cached). | Model-selection ladder documented per feature; explanation templates cover all reason codes; the LLM-polish path is flag-gated and cost-tracked. | `recommendation`, `media` | P09, P11 | 10 |
...
| §4.1 | Mobile application and 3D surface choice | N-PERF-070, R-AVA-100/120; doc 05 (RN+Expo+Filament decision) |
