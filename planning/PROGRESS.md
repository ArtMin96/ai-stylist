...
- **Not done / next action:** fill `dev.enc.yaml` values only as the corresponding service accounts are provisioned; create staging/prod files in P03 when those environments exist. Back up the developer identity in the team's approved password manager so loss of this workstation does not remove developer access; during the review follow-up no keys file was present at `~/.config/sops/age/keys.txt` on this workstation, so first confirm where the developer identity actually lives. Then label the two recipients in `.sops.yaml` (developer vs `ci`) with a comment line above each key, as the file header now asks.
