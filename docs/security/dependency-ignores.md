...
- Ignores live in two places that must stay in sync:
  ...
  - `pnpm.auditConfig.ignoreGhsas` in the root `package.json`, the same id list, because
    ...
    them.
  - osv-scanner (2.5.1) applies only the `osv-scanner.toml` in the **same directory** as the
...
is the latest release, and Kotlin 2.4.20 and Gradle 9.7.1 are the latest GA releases. So they are
