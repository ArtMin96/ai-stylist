...
Public interface: `index.ts` only. P02 skeleton: the sole export is the empty NestJS `MediaModule` class (composition-root wiring, imported by `tests/media.smoke.test.ts`); no domain service, command, query, type, or port is exported yet. Everything under `internal/` is blocked by the `public-api-only` boundary rule (`just arch-check`, `just lint`).
...
- Never calls pg-boss, R2, or fal.ai SDKs directly; async side effects go through the Postgres outbox (doc 04 §9).
