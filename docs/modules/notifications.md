...
> Module names are canonical per [SPINE §3](../../planning/SPINE.md). Path: `apps/api/src/modules/notifications`. This contract is the module's source of truth; code that contradicts it is wrong until a DEC entry says otherwise.

- **Responsibility (one sentence):** Push delivery, scheduling, and notification preferences.
- **Owner:** @team (placeholder — see `CODEOWNERS`) · **Status:** skeleton (P02) · **Last updated:** 2026-09-13

## Public interface

Public interface: `index.ts` only. P02 skeleton: the sole export is the empty NestJS `NotificationsModule` class (composition-root wiring, imported by `tests/notifications.smoke.test.ts`); no domain service, command, query, type, or port is exported yet. Everything under `internal/` is blocked by the `public-api-only` boundary rule (`just arch-check`, `just lint`).
...
None yet (P02 skeleton). Planned per SPINE §3: `notification_prefs`, `deliveries`. Table definitions, when they exist, live in `apps/api/src/modules/notifications/internal/schema.ts` and are composed from `packages/db/` (ADR-0001 §3).
...
- Public API of `identity`; `packages/shared-kernel`. Push transport (FCM/APNs) via a port implemented in `platform`.
...
- Never imports FCM or APNs SDKs; delivery payloads contain no sensitive data classes (doc 11).

## Tests

- Location: `apps/api/src/modules/notifications/tests/`
...
- New channels are new port implementations; new notification kinds are declared in `packages/contracts`.
