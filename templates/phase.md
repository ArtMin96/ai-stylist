...
> File name: `planning/phases/P##-<slug>.md` per [SPINE §5](../planning/SPINE.md). Every section below is REQUIRED (brief §10); write "None" explicitly rather than deleting a section. Status values per [PROGRESS.md](../planning/PROGRESS.md).

## 1. Overview

- **Phase:** P## — <name>
- **Status:** `NOT_STARTED` _(mirror of PROGRESS.md; PROGRESS.md wins on conflict)_
- **Goal (one sentence):** <…>
...
IDs from [01-requirements-and-traceability.md](../planning/01-requirements-and-traceability.md) only.
...
Module names per [SPINE §3](../planning/SPINE.md); update each touched module's contract file.
...
For each AI use in this phase: classification (per brief §3.1), why deterministic code is insufficient, contract, fallback, cost budget — or link to the owning row in [10-ai-usage-cost-and-evaluation.md](../planning/10-ai-usage-cost-and-evaluation.md).
...
- New sensitive data introduced + classification (per [11](../planning/11-security-privacy-and-compliance.md)): <…>
...
- Product analytics events (taxonomy per [14](../planning/14-observability-operations-and-analytics.md)): <…>
...
Link RISK-NN/ASM-NN in [16](../planning/16-risks-open-questions-and-decision-log.md); add phase-local ones there, not here.
...
- [PROGRESS.md](../planning/PROGRESS.md): set status per its rules; `DONE` only with §20 evidence.
