...
- **Goal (one sentence):** Build a vertical RN + `react-native-filament` prototype — morphing Anny-derived avatar, 3–4 poses, rotate/zoom, camera→local-storage batch capture — and measure it on real iOS and Android hardware to produce a formal **go/no-go decision** for the entire 3D surface.
...
- **Why now:** RISK-01 is the #1 ranked risk: no consumer app is publicly proven on `react-native-filament` at scale. Every avatar phase (P04, P05, P10) and the mobile stack choice itself sit on this assumption (ASM-01/ASM-02, RB-5). The brief (§4 intro) and [05 §2.3](../05-technology-decisions.md) mandate a real-device prototype gate before product code. Runs after P00 (ratified gate criteria); may run in parallel with P02.
...
- External blockers:
  ...
  - **Apple Developer Program + Play Console accounts** and **EAS free tier** access (G7 requires TestFlight + Play internal-track delivery; r1).
  - RISK-01 / RISK-06 / RISK-09 are the risks under test, not blockers.

## 4. In scope / out of scope

**In scope:**
- A standalone Expo (SDK 55+, prebuild/dev-client, New Architecture) prototype app in `prototype/p01-filament/` — **spike code, explicitly disposable**; findings and asset tooling survive, code does not get promoted wholesale.
...
- Any product/monorepo code, module structure, CI wiring → P02 (except the EAS build config learnings handed to ADR-P02).
...
| `platform` (future) | EAS build behavior with prebuild + native Filament module measured (feeds ADR-P02 / A3–A5) | No |
...
|---|---|
| Mobile | Expo prebuild app; Filament scene (load/morph/pose/orbit/zoom); capture grid; SQLite index; measurement HUD + trace export |
| Backend | None |
| Workers (ML/media) | None |
| Data / migrations | Local SQLite only (disposable) |
| Infrastructure | EAS project (free tier); Apple/Play app records for internal distribution; Git LFS for prototype assets ([05 §3.4](../05-technology-decisions.md)) |
...
|---|---|---|---|
| P01-T01 | Workspace bring-up: `prototype/p01-filament/` Expo SDK 55+ prebuild app, TypeScript, New Architecture, `react-native-filament` (pin v1.11.0-era + Filament 1.76-era; record exact pins), runs on both dev devices via dev-client | — | 1 |
| P01-T02 | Anny asset spike: Blender-headless export of one Anny base → `.glb` with ≥4 morph targets + skeleton; gltfpack (Draco/meshopt) + glTF-Transform (KTX2, morph pruning); document mesh stats + conventions vs [07 §9](../07-3d-avatar-and-garment-pipeline.md) | — | 2 |
...
| P01-T04 | Filament scene: load GLB, IBL + PBR setup, morph-weight animation loop (≥4 blend shapes), pose switching | P01-T01, P01-T02 | 2 |
...
| P01-T07 | Measurement harness: fps/frame-time capture, RSS/GPU memory sampling, cold-start timer, JSON trace export + collation script | P01-T04 | 1 |
| P01-T08 | Delivery lane (G7): EAS build profiles; TestFlight + Play internal track from the same codebase; record build times/cost/flakes for ADR-P02 (A3/A5 evidence) | P01-T04, P01-T06 | 2 |
...
| P01-T10 | Asset-diet pass **only if** G5/G6 red (A2 path): LOD/KTX2/Draco tuning + Expo Atlas tree-shaking; re-run affected gates; both runs kept in the report | P01-T09 | 1 |
...
| Viewer app | Morph/pose state logic (pure TS) | None | None | None (Filament rendering is validated on-device, not mocked) | **G1–G3, G5–G6 device runs are the test**; traces archived |
...
| Performance — G5 package size | **< 100 MB** IPA and APK/AAB | EAS artifact sizes from the G7 builds |
...
| Cost | Prototype spend ≤ EAS free tier + existing dev accounts (≈ $0 marginal; iOS lane envelope $30–50/mo begins P02) | EAS/GHA billing pages, screenshots archived |
...
- Risks in play: **RISK-01** (`react-native-filament` maturity — the risk under test), **RISK-06** (Anny production-asset readiness — A6 spike), **RISK-09** (low-end Android — first low-tier data), **RISK-12** (iOS delivery lane — G7 exercises it, feeds ADR-P02). Assumptions under test: **ASM-01, ASM-02** (partial), **A1–A3, A6** ([05 §8](../05-technology-decisions.md)); this phase *is* **RB-5**.
- **Stop/kill criteria for this phase (the formal go/no-go gate):**
  ...
  - **NO-GO:** any of G1–G3 fail after one honest tuning iteration, or G5/G6 fail structurally after the T10 asset diet, or a blocking `react-native-filament` defect has no upstream fix path. → execute the **pivot ladder** from [05 §2.3](../05-technology-decisions.md), in order, each rung an ADR:
    1. Wrapper failure, Filament fine → keep RN, drive native Filament via a JSI bridge.
    2. RN/3D integration failure on one platform → platform-native 3D view (RealityKit iOS / Filament Android) embedded in RN behind the renderer boundary.
    3. Systemic RN failure → fully native Swift/Kotlin; re-scope phases (2× cost accepted consciously).
    4. Flutter reconsidered **only** if flutter_filament has reached stable by pivot time.
...
1. Install the TestFlight build on the iPhone 13-class device and the Play-internal build on the Galaxy A52-class device (both from the same commit — G7).
...
- AC-5: G5/G6 — IPA and APK/AAB artifact sizes < 100 MB each (EAS artifact listing screenshot); RSS < 300 MB sampled during the AC-1 run on both mid-tier devices; low-tier device measured and recorded (pass or documented fallback trigger per [07 §10.2](../07-3d-avatar-and-garment-pipeline.md)).
...
- AC-9: G7 — both store builds (TestFlight + Play internal) came from the same commit with zero platform-specific workarounds in the core 3D loop (diff-audit note in the report); EAS cost/flake data recorded for ADR-P02.
...
pnpm test                      # prototype unit/integration tests — all pass, no skips
pnpm lint && pnpm typecheck    # clean
node tools/validate-asset.mjs assets/avatar.glb   # glTF validator + KTX2/Draco checks green
...
Evidence to attach/link: gate report with per-device measurement tables; raw trace files; EAS build artifacts + size screenshots; TestFlight/Play-internal build IDs; the DEC entry. Never fabricated, never emulator-sourced.

## 21. Documentation and PROGRESS.md updates

- Docs to update: [doc 16](../16-risks-open-questions-and-decision-log.md) (DEC go/no-go, RISK-01/-06/-09 status, OQ-08 data); [doc 07 §9–§10](../07-3d-avatar-and-garment-pipeline.md) (conventions confirmed/corrected, measured budget baselines); [doc 13 §12.1](../13-testing-quality-and-performance.md) (P01-marked rows get measured values, labels flipped from hypothesis); [doc 05](../05-technology-decisions.md) (A1–A3/A6 assumption outcomes); ADR-P02 input note (EAS evidence).
