...
#   packages/shared-kernel/registry/*.json  ──tools/codegen/gen-kernel.mjs kotlin──▶
#   packages/contracts/gen/kotlin-client/kernel/src/main/kotlin/app/aistylist/contracts/kernel/*.kt
...
# shared-kernel registries (reason codes, entitlements, units) so Kotlin never hand-copies them
# (CLAUDE.md single source of truth; OQ-15). Plain Kotlin, no dependencies; banner is per file.
"${node_cmd[@]}" "$REPO_ROOT/tools/codegen/gen-kernel.mjs" kotlin \
  "$staged/kernel/src/main/kotlin/app/aistylist/contracts/kernel" >/dev/null
